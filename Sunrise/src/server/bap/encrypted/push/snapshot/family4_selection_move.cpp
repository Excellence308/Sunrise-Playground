...
/** Builds one resident subclass item upsert from an uncommitted ability selection. */
bool prepare_subclass_selection(Scratch& scratch,
    ...
    for (std::size_t index = 0; index < selected.loadout.itemCount; ++index) {
        ...
        if (changed.itemCount != 0 || !item.equipped
            ...
            || mutation.selectedEntry >= item.instance.socketEntryCount
            || item.instance.socketEntryStates[mutation.requestedEntry]
                   != family4_datagen::instance::SocketEntryState::active
            || item.instance.socketEntryStates[mutation.selectedEntry]
                   != family4_datagen::instance::SocketEntryState::active) {
    ...
    const int count = std::snprintf(
        ...
        "character=0x%llX instance=0x%llX item_definition=%u target_definition=%u "
        "socket_list=%u requested_entry=%u selected_entry=%u group=%u objects=1",
        selection.after.family4Version,
