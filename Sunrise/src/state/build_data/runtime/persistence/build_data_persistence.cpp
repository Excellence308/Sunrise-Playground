...
cache::records::MutableDomains scratch_domains(Context& state) noexcept {
    ...
    const auto vendorDefinitions = ensure_scratch<vendors::Definition, vendors::kDefinitionCapacity>(
...
cache::records::Domains occupied_domains(Context& state,
    ...
    return {
        ...
        std::span<const vendors::IndexEntry>{state.vendorIndexScratch.get(), counts.vendorIndex},
        std::span<const vendors::Definition>{state.vendorDefinitionScratch.get(),
                                             counts.vendorDefinitions},
        std::span<const vendors::SaleRow>{state.vendorSaleRowScratch.get(), counts.vendorSaleRows},
        std::span<const vendors::InstalledRow>{state.vendorInstalledRowScratch.get(),
                                               counts.vendorInstalledRows},
    };
}
